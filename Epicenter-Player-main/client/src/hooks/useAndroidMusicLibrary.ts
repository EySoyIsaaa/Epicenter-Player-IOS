...
export function useAndroidMusicLibrary() {
  ...
  const [isNativeMusicPlatform, setIsNativeMusicPlatform] = useState(false);
  const [platformName, setPlatformName] = useState<'android' | 'ios' | 'web'>('web');
  ...
  useEffect(() => {
    ...
    const capacitorPlatform = (window as any)?.Capacitor?.getPlatform?.() as string | undefined;
    const isAndroidDevice = capacitorPlatform === 'android' || /android/.test(userAgent);
    const isIOSDevice = capacitorPlatform === 'ios' || /iphone|ipad|ipod/.test(userAgent);

    logger.debug('🔍 [MusicLibrary] UserAgent:', userAgent);
    logger.debug('🔍 [MusicLibrary] Capacitor platform:', capacitorPlatform);

    if (isAndroidDevice) {
      setPlatformName('android');
      setIsNativeMusicPlatform(true);
      return;
    }
    ...
    setPlatformName('web');
    setIsNativeMusicPlatform(false);
  }, []);
  ...
  const scanMusicLibrary = async (): Promise<AndroidMusicFile[]> => {
    if (!isNativeMusicPlatform) {
      const errorMsg = 'Esta funcionalidad solo está disponible en apps nativas (Android/iOS)';
      setError(errorMsg);
